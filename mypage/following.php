...
$allusers = "";
$loginid = $userinfo["id"];
$users = $pdo->prepare("SELECT * FROM relationship WHERE followinguser='$loginid'");
$usersstatus = $users->execute(); //実行
if($usersstatus==false) {
    sql_error();
}else{
    while( $result = $users->fetch(PDO::FETCH_ASSOC)){
        $followingid = $result["followeduser"];
        $user = $pdo->prepare("SELECT * FROM user WHERE id='$followingid'");
