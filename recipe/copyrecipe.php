...
$authorsql = "SELECT * FROM users WHERE id=:userid";
$author = $pdo->prepare($authorsql);
$author->bindValue(':userid', $recipe["userid"], PDO::PARAM_INT);
$authorstatus = $author->execute();

if($authorstatus==false){
    sql_error();
} else {
    $recipeauthor = $author->fetch();
}
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
      <form action="../recipe/savecopyrecipe.php?id=<?=$recipeid?>" method="post">
        <h1 class="h3 mb-3 font-weight-normal"><?=$recipeauthor["name"]?>さんのRecipe</h1>
        <input type="hidden" name="author" value="<?=$recipeauthor["id"]?>">
